...
const handleToContact = async (req, res) => {
    try {
      ...
      const requiredFields = ['firstName', 'lastName', 'email', 'mobile', 'message'];
  ...
  const getAllContacts = async (req, res) => {
    try {
      const contacts = await Contact.find({ status: "Active" }).sort({ updatedOn: -1 }); 
      const totalInquiryUsers = await Contact.countDocuments({ status: "Active" });
